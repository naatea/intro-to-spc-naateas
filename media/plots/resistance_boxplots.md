...
p_pressure <- ggplot(df_machine1, aes(x = Pressure, y = PartResistance, fill = Pressure)) +
  geom_boxplot() +
  geom_hline(yintercept = USL, linetype = "dashed", color = "red", size = 1, alpha = 0.7) + # Add USL
  labs(
...
combined_plot <- subplot(ggplotly(p_pressure), ggplotly(p_temperature), nrows = 1, shareY = TRUE) %>% 
  layout(title = list(text = 'Part Resistance Distribution by Pressure and Temperature (Machine 1) with USL', font = list(size = 20)),
         annotations = list(
